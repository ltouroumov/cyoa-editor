...
Overall: **viewer 69/103 · editor 21/104**
...
## Choice "functions" (on-select / on-deselect actions) (viewer: 3/8; editor: 0/8)
...
- Reset all selections on select (`cleanACtivatedOnSelect`) — viewer: [Done] · editor: [Missing] — `setSelected` seeds the pipe from an empty selection when a flagged choice is selected interactively, so only that choice and its `activateOtherChoice` targets survive; derived state (points, allowed-choice deltas, multi-use counts) resets for free. Bulk build/code import bypasses it, matching legacy
